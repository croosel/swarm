...
| 排程蜂（Orchestrator） | Qwen 3.7 Plus + 分解覆核檢查 | DAG 任務圖生成 + 耦合度判斷 | 3-Phase 主 Agent |
...
| 裁判蜂（Judge） | 非執行模型交叉驗證 + 加權融合 | 不參與執行，獨立審計 | OPC 質檢總監 |
...
class FastTrackGate:
    ...
    FAST_TRACK_TEMPLATES = {
        "translation":        {"model": "qwen-37-plus",    "reason": "中文理解強"},
        "summarization":      {"model": "minimax-m27",     "reason": "長文本處理"},
        "code_snippet":       {"model": "deepseek-v4",     "reason": "程式碼生成"},
        "structured_extract": {"model": "qwen-37-plus",    "reason": "結構化輸出"},
        "reasoning_step":     {"model": "deepseek-v4",     "reason": "推理最深"},
    }
...
| 角色               | 模型             | 負責的任務類型           |
+====================+==================+=========================+
| 排程蜂 (Orch.)     | Qwen 3.7 Plus    | 任務拆解、結構化分析     |
| 長文蜂 (LongDoc)   | MiniMax M2.7     | 超長上下文（1M）、文件分析|
| 碼蜂 (Coder)       | DeepSeek V4      | 程式碼生成/修改/審查     |
| 結構蜂 (Struct)    | Qwen 3.7 Plus    | 結構化輸出、JSON、表格   |
| 裁判蜂 (Judge)     | 非執行模型       | 交叉驗證紅隊、最終綜合   |
| 推理蜂 (Reasoner)  | DeepSeek V4      | 數學、邏輯、深度分析     |
+====================+==================+=========================+
```

成本估算（以程式碼審查為例）：
- Orchestrator 分解 (Qwen 3.7 Plus): ~500 tokens out → ¥0.003
- MiniMax M2.7 靜態分析: ~2K in, ~1K out → ¥0.014
- DeepSeek V4 邏輯審查: ~2K in, ~1K out → ¥0.008
- Qwen 3.7 Plus 綜合: ~3K in, ~1.5K out → ¥0.021
- **總計: ~¥0.046 / 次**（對比單模型 Qwen 3.7 Plus ~¥0.033，約 1.4 倍）
- **成本紅線**: 單次任務不超過 ¥0.50
...
```python
AVAILABLE_MODELS = {"qwen-37-plus", "minimax-m27", "deepseek-v4"}

def _pick_verifier(worker_model: str, default_judge: str) -> str:
    """驗證模型必須與生產模型不同。3 模型陣容下總能找到合適驗證者。"""
    if worker_model != default_judge:
        return default_judge
    # worker 與 default_judge 相同時，選另外兩個模型中任一
    return next(m for m in AVAILABLE_MODELS if m != worker_model)
```
...
| 生產模型 | 驗證模型 |
|---------|---------|
| qwen-37-plus | minimax-m27 |
| minimax-m27 | deepseek-v4 |
| deepseek-v4 | qwen-37-plus |
...
providers:
  deepseek-v4:
    ...
    strengths: [reasoning, cost_efficiency, code]
    pricing:
  ...
  qwen-37-plus:
    ...
    context_window: 131072
    strengths: [structured_output, agent_tool_calling, reasoning]
    pricing:
      input_per_million_cny: 4.0
      output_per_million_cny: 12.0
  minimax-m27:
    api_base: "https://api.minimax.chat"
    api_key_env: "MINIMAX_API_KEY"
    litellm_model: "openai/minimax-m27"
    context_window: 1048576
    strengths: [long_context, agent_reasoning, document_understanding]
    pricing:
...
```yaml
defaults:
  scout_model: "qwen-37-plus"
  judge_model: "qwen-37-plus"
  default_model: "deepseek-v4"

type_routing:
  reasoning:  { primary: "deepseek-v4",   fallback: "qwen-37-plus" }
  code:       { primary: "deepseek-v4",   fallback: "qwen-37-plus" }
  analysis:   { primary: "minimax-m27",   fallback: "deepseek-v4" }
  creative:   { primary: "qwen-37-plus",  fallback: "minimax-m27" }
  data:       { primary: "qwen-37-plus",  fallback: "minimax-m27" }

verification:
  rule: "verifier_model != worker_model"
  default_verifier: "qwen-37-plus"
  overrides:
    "qwen-37-plus": "minimax-m27"
    "minimax-m27": "deepseek-v4"
    "deepseek-v4": "qwen-37-plus"
...
| 成本倍數 | Architect: 1.8x vs Critic: 5x | **1.4x（含 Fast-Track）** | Architect 的估算是基於實際 token 計數。Critic 的 5x 是不含 Fast-Track 的最壞情況。80% 簡單任務走 Fast-Track 後，平均約 1.5x。 |
...
5. **成本合理性**：平均任務成本 ~¥0.046（含 Fast-Track 後更低），月成本可控。 ✓

---

**文件狀態：v1.1 定稿。模型陣容更新為 3 模型分工（Qwen 3.7 Plus / MiniMax M2.7 / DeepSeek V4），整合偵查蜂、建築蜂、工程蜂、審稿蜂四蜂產出，交叉驗證完成。**
